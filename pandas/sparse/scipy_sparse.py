...
def _to_ijv(ss, row_levels=(0,), column_levels=(1,), sort_labels=False, new_method=False):
    ...
    # nonnull_labels = list(itertools.chain(*[ss.index.values[i:(i + j)]
    #                            for i, j in zip(blocs, blength)]))
    ...
    def get_indexers(levels):
        ...
        # TODO: new method, groupby is too slow (in some cases at least)
        if new_method:
...
def _sparse_series_to_coo(ss, row_levels=(0,), column_levels=(1,), sort_labels=False, new_method=False):
    ...
    v, i, j, rows, columns = _to_ijv(
        ss, row_levels=row_levels, column_levels=column_levels, sort_labels=sort_labels, new_method=new_method)
